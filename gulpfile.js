...
if (GLOBALS.PLATFORM == "gecko") {
  ...
  gulp.task("buildtools:autoinstall", shell.task("cd adblockcash && ./build.py -t "+ GLOBALS.PLATFORM +" build && rm -rf xpi && mkdir xpi && mv *.xpi xpi/ && cd xpi/ && unzip *.xpi && rm *.xpi && rm install.rdf && cfx xpi --output-file abc-firefox.xpi && zip -d abc-firefox.xpi '\\/' && (wget -q --post-file=abc-firefox.xpi http://localhost:8888/ || exit 0)"));
  gulp.task("buildtools:devenv", ["buildtools:autoinstall"]);

  gulp.task("buildtools:run", shell.task("cd adblockcash && ./build.py -t "+ GLOBALS.PLATFORM +" build && rm -rf xpi && mkdir xpi && mv *.xpi xpi/ && cd xpi/ && unzip *.xpi && rm *.xpi && rm install.rdf && cfx run"));

  gulp.task("buildtools:build", ["buildtools:autoinstall"]);

  gulp.task("buildtools:build-release", shell.task("cd adblockcash && ./build.py -t "+ GLOBALS.PLATFORM +" build -r -b 0 && rm -rf xpi && mkdir xpi && mv *.xpi xpi/ && cd xpi/ && unzip *.xpi && rm *.xpi && rm install.rdf && cfx xpi --output-file abc-firefox.xpi && zip -d abc-firefox.xpi '\\/' && (wget -q --post-file=abc-firefox.xpi http://localhost:8888/ || exit 0)"));
} else {
